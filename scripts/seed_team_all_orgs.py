#!/usr/bin/env python3
"""Carga el equipo piloto (6 empleados) en todas las empresas activas."""
from __future__ import annotations

import sys
from datetime import date
from decimal import Decimal
from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(ROOT / "src"))

from epayroll.db.connection import get_connection
from epayroll.db.legal_config_repository import LegalConfigRepository
from epayroll.db.repositories import ContractRepository, EmployeeRepository

# ficha, nombres, apellidos, cedula, salario_mensual, bono_mensual_ref
TEAM = [
    ("1", "Juan", "Perez", "6-69-13", Decimal("1700.00"), Decimal("0")),
    ("2", "Edwin", "Lopez", "8-937-952", Decimal("825.00"), Decimal("100.00")),
    ("3", "Davis", "Fernandez", "3-738-253", Decimal("825.00"), Decimal("150.00")),
    ("4", "Abigail", "Govea", "8-993-1252", Decimal("650.00"), Decimal("100.00")),
    ("5", "Ibania", "Baltodano", "E-8-93577", Decimal("650.00"), Decimal("100.00")),
    ("6", "Jose", "de la Rosa Barrios", "PENDIENTE-6", Decimal("650.00"), Decimal("0")),
]


def list_active_organizations() -> list[tuple[str, str]]:
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT id, razon_social
                FROM organizations
                WHERE activo = true
                ORDER BY razon_social
                """
            )
            return [(str(r[0]), r[1]) for r in cur.fetchall()]


def main() -> None:
    legal = LegalConfigRepository()
    emp_repo = EmployeeRepository()
    contract_repo = ContractRepository()
    inicio = date(2026, 1, 1)
    orgs = list_active_organizations()
    if not orgs:
        print("No hay empresas activas.")
        return

    total_created = 0
    total_skipped = 0

    for org_id, org_name in orgs:
        legal.seed_org_defaults(org_id)
        created = 0
        skipped = 0
        print(f"\n{org_name} ({org_id})")
        for ficha, nombres, apellidos, cedula, salario, bono in TEAM:
            if emp_repo._cedula_exists_in_org(org_id, cedula):
                print(f"  · {cedula} {nombres} {apellidos} — ya existe, omitido")
                skipped += 1
                continue
            nota = f"Bono mensual ref.: B/. {bono}" if bono > 0 else None
            emp = emp_repo.create(
                organization_id=org_id,
                cedula=cedula,
                nombres=nombres,
                apellidos=apellidos,
                ficha=ficha,
                direccion=nota,
            )
            contract_repo.create(
                employee_id=emp.id,
                contract_type_codigo="INDEFINIDO",
                salario_base=salario,
                fecha_inicio=inicio,
                forma_pago="QUINCENAL",
            )
            print(f"  ✓ Ficha {ficha}: {nombres} {apellidos} — B/. {salario}/mes")
            created += 1
        total_created += created
        total_skipped += skipped
        print(f"  → {created} nuevos, {skipped} omitidos")

    print(f"\nListo: {total_created} empleados creados en {len(orgs)} empresa(s), {total_skipped} omitidos.")


if __name__ == "__main__":
    main()
