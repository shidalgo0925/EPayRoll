...
class LiquidacionPDF(FPDF):
    def footer(self) -> None:
        ...
        self.cell(0, 10, _safe_text(f"Liquidacion EPayRoll — Pagina {self.page_no()}"), align="C")


def generate_liquidation_pdf(data: dict[str, Any], output: Path | BinaryIO) -> None:
    """Genera PDF de liquidacion laboral."""
    pdf = LiquidacionPDF()
    ...
    pdf.cell(0, 10, _safe_text("LIQUIDACION LABORAL"), new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    pdf.set_font("Helvetica", "", 11)
    pdf.set_text_color(60, 60, 60)
    pdf.cell(0, 6, _safe_text(data.get("organization_nombre") or ""), new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    pdf.ln(4)
    ...
    if data.get("cedula"):
        pdf.cell(0, 6, _safe_text(f"Cedula: {data['cedula']}"), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.cell(0, 6, _safe_text(f"Causa: {data.get('causa', '—')}"), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.cell(
        0,
        6,
    ...
    if data.get("case_id"):
        pdf.cell(0, 6, _safe_text(f"Caso: {data['case_id']}"), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.ln(4)
    ...
    if lines:
        ...
        pdf.cell(0, 7, _safe_text("Monto"), new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="R", fill=True)
        pdf.set_text_color(0, 0, 0)
        pdf.set_font("Helvetica", "", 10)
        for line in lines:
            label = line.get("concepto") or line.get("descripcion") or "—"
            pdf.cell(130, 7, _safe_text(label))
            pdf.cell(0, 7, _money(line.get("monto")), new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="R")
    else:
        pdf.set_font("Helvetica", "B", 10)
    ...
    pdf.multi_cell(
        ...
        4,
        _safe_text(
            "Documento informativo de liquidacion laboral (GT-05/GT-06). "
